...
% Define colors %
%%%%%%%%%%%%%%%%%
\definecolor{pfasstCoarseColor}{RGB}{175,90,80}
\definecolor{pfasstFineColor}{RGB}{110,159,189}
...
\tikzmath{
  ...
  function legend(\pfasstSteps) {
      ...
      \yl = (\ya+\yb)/2;
      { \node[text=pfasstTextColor, align=left,text width=\legendLabelWidth em] at (\xl em,\yl em) {Coarse Sweep}; };
      ...
      \xl = \xb+\legendBlockHSpace;
      \yl = (\ya+\yb)/2;
      { \node[text=pfasstTextColor, align=left,text width=\legendLabelWidth em] at (\xl em,\yl em) {Fine Sweep}; };
      \ya = \ya-\legendBlockHeight-\legendBlockVSpace;
      \yb = \yb-\legendBlockHeight-\legendBlockVSpace;
      drawCoarseComm(\xa, (\ya+\yb)/2, \xb, (\ya+\yb)/2);
      \xl = \xb+\legendBlockHSpace;
      \yl = (\ya+\yb)/2;
      { \node[text=pfasstTextColor, align=left,text width=\legendLabelWidth em] at (\xl em,\yl em) {Coarse Comm}; };
      \ya = \ya-\legendBlockHeight-\legendBlockVSpace;
      \yb = \yb-\legendBlockHeight-\legendBlockVSpace;
      drawFineComm(\xa, (\ya+\yb)/2, \xb, (\ya+\yb)/2);
      \xl = \xb+\legendBlockHSpace;
      \yl = (\ya+\yb)/2;
      { \node[text=pfasstTextColor, align=left,text width=\legendLabelWidth em] at (\xl em,\yl em) {Fine Comm}; };
  };
}
